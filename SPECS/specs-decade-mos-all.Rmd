...
Use `purl()` to generate a plain R-script.
  ...
  for (i in 1:n) {
    ...
    if (attr(X,'member_by')=='month')
      index(y) <- as.Date(paste(year(y),i,'01',sep='-'),
                          origin = as.Date("1950-01-01")) else
      index(y) <- as.Date(as.Date(paste(year(y),'01-01',sep='-'))+i,
                          origin = as.Date("1950-01-01"))
...
mannualfcst <- function(x,l=1,it=NULL,is=NULL,mask=FALSE,verbose=FALSE) {
  ...
  if (attr(X,'member_by')=='month') {
    ...
    jday <- julian(index(x))-julian(as.Date(paste(year(x),'01-01',sep='-')))
    m <- as.integer(rownames(table(jday)))
    if (verbose) print(paste('separate ensemble members by day 1 --',m))
    j <- 1
    for (i in m) {
      itday <- is.element(jday,i)
      if (verbose) print(c(i,j,sum(itday)))
      y <- pentad(subset(X,it=itday),l=l)
      index(y) <- as.Date(paste(year(y),i,'01',sep='-'))
      if (j==1) Y <- zoo(y) else
                Y <- c(Y,zoo(y))
      j <- j+1
...
```{r compare}
comparemannual <- function(tas_DECA,reanalysis='~/R/air.mon.mean.nc',mask=TRUE,l= 3,eofs=NULL) {
...
diagnoseMOS <- function(results,y,l=1) {
  ...
  print(paste('zmos <-',substr(results,zl+1,nchar(results)-4)))
...
if (!file.exists('~/storeA/mu_0.25deg_reg.nc'))
...
## Carry out a LOO cross-validation experiment
  ...
  if (!file.exists('~/R/specsdecadal.z.fw.y1.rda')) {
    print('do specsdecadal.z.fw.y1.rda')
    z.fw.y1 <- D53Exp(X=psl_DECA,Y=fw.eobs,m=1,it=NULL,is=NULL,mask=FALSE)
    save(file='~/R/specsdecadal.z.fw.y1.rda',z.fw.y1)
    rm("z.fw.y1"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.mu.y1.rda')) {
    print('do specsdecadal.z.mu.y1.rda')
    z.mu.y1 <- D53Exp(X=tas_DECA,Y=mu.eobs,m=1,it=NULL,is=NULL,mask=TRUE)
    save(file='~/R/specsdecadal.z.mu.y1.rda',z.mu.y1)
    rm("z.mu.y1"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.fw.y1.3.rda')) {
    print('do specsdecadal.z.fw.y1.3.rda')
    z.fw.y1.3 <- D53Exp(X=psl_DECA,Y=fw.eobs,m=1:3,it=NULL,is=NULL,mask=FALSE)
    save(file='~/R/specsdecadal.z.fw.y1.3.rda',z.fw.y1.3)
    rm("z.fw.y1.3"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.mu.y1.3.rda')) {
    print('do specsdecadal.z.mu.y1.3.rda')
    z.mu.y1.3 <- D53Exp(X=tas_DECA,Y=mu.eobs,m=1:3,it=NULL,is=NULL,mask=TRUE)
    save(file='~/R/specsdecadal.z.mu.y1.3.rda',z.mu.y1.3)
    rm("z.mu.y1.3"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.fw.y1.5.rda')) {
    print('do specsdecadal.z.fw.y1.5.rda')
    z.fw.y1.5 <- D53Exp(X=psl_DECA,Y=fw.eobs,m=1:5,it=NULL,is=NULL,mask=FALSE)
    save(file='~/R/specsdecadal.z.fw.y1.5.rda',z.fw.y1.5)
    rm("z.fw.y1.5"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.mu.y1.5.rda')) {
    print('do specsdecadal.z.mu.y1.5.rda')
    z.mu.y1.5 <- D53Exp(X=tas_DECA,Y=mu.eobs,m=1:5,it=NULL,is=NULL,mask=TRUE)
    save(file='~/R/specsdecadal.z.mu.y1.5.rda',z.mu.y1.5)
    rm("z.mu.y1.5"); gc(reset=TRUE)
  }
  if (!file.exists('~/R/specsdecadal.z.fw.y1.9.rda')) {
    print('do specsdecadal.z.fw.y1.9.rda')
     z.fw.y1.9 <- D53Exp(X=psl_DECA,Y=fw.eobs,m=1:9,it=NULL,is=NULL,mask=FALSE)
     save(file='~/R/specsdecadal.z.fw.y1.9.rda',z.fw.y1.9)
     rm("z.fw.y1.9"); gc(reset=TRUE)
   }
  if (!file.exists('~/R/specsdecadal.z.mu.y1.9.rda')) {
    print('do specsdecadal.z.mu.y1.9.rda')
    z.mu.y1.9 <- D53Exp(X=tas_DECA,Y=mu.eobs,m=1:9,it=NULL,is=NULL,mask=TRUE)
    save(file='~/R/specsdecadal.z.mu.y1.9.rda',z.mu.y1.9)
    rm("z.mu.y1.9"); gc(reset=TRUE)
  }

## Diagnostics:
  print('diagnose the MOS results')
  load('~/R/specsdecadal.z.mu.y1.3.rda')
  it0 <- range(year(z.mu.y1.3))
  print(paste('time period',paste(it0,collapse='-')))
  score.mu.y1 <- diagnoseMOS('~/R/specsdecadal.z.mu.y1.rda',mu.eobs)
  score.fw.y1 <- diagnoseMOS('~/R/specsdecadal.z.fw.y1.rda',fw.eobs)
  score.mu.y1.3 <- diagnoseMOS('~/R/specsdecadal.z.mu.y1.3.rda',mu.eobs,l=3)
  score.fw.y1.3 <- diagnoseMOS('~/R/specsdecadal.z.fw.y1.3.rda',fw.eobs,l=3)
  score.mu.y1.5 <- diagnoseMOS('~/R/specsdecadal.z.mu.y1.5.rda',mu.eobs,l=5)
  score.fw.y1.5 <- diagnoseMOS('~/R/specsdecadal.z.fw.y1.5.rda',fw.eobs,l=5)
  score.mu.y1.9 <- diagnoseMOS('~/R/specsdecadal.z.mu.y1.9.rda',mu.eobs,l=9)
  score.fw.y1.9 <- diagnoseMOS('~/R/specsdecadal.z.fw.y1.9.rda',fw.eobs,l=9)
...
if (diagnoseFCST) {
  ...
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 1)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 2)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 3)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 4)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 5)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 6)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 7)
  comparemannual(psl_DECA,reanalysis='~/R/slp.mon.mean.nc',mask=FALSE,l= 8)
...
t2m <- retrieve('~/R/air.mon.mean.nc',lon=c(-90,60),lat=c(-20,80))
t2m <- anomaly(t2m)
...
## Repeat for SLP
slp <- retrieve('~/R/slp.mon.mean.nc',lon=c(-90,60),lat=c(-20,80))
slp <- anomaly(slp)
