...
wetdaymean <- function(ifile,cline='cdo yearmean',fname='data/wetdaymean.nc') {
  print('calculate mask')
  system(paste('cdo gtc,1',ifile,'data/mask.nc'))
  ## her blir ofile en mask med 1 i alle tidsskritt med data over konstanten (satt til 0),
  ## og verdi 0 i alle andre
  print('apply mask')
  system(paste('cdo ifthen data/mask.nc',ifile, fname))
...
D53Exp <- function(X,Y,m=c(1,3),it=NULL,is=NULL,mask=FALSE,verbose=TRUE) {
  ...
  map(rmap,colbar=list(breaks=seq(-1,1,by=0.1),pal='t2m',rev=TRUE),type='fill')
  invisible(XVAL)
}
...
comparemannual <- function(tas_DECA,reanalysis='air.mon.mean.nc',mask=TRUE,l= 3,eofs=NULL) {
...
## Compare the multi-annual scale ensemble mean forecast with the NCEP/NCAL reanalysis
  ...
  dev.copy2pdf(file=paste('figSM.D53Exp.corfield-',varid,land,'-',l,'-yrs.pdf',sep=''))
## Generate a common EOF
  ...
  dev.copy2pdf(file=paste('figSM.D53Exp.ceof-',varid,land,'-',l,'-yrs.pdf',sep=''))
  invisible(rmap)
## Generate a common EOF
}
...
diagnoseMOS <- function(results,y,l=1) {
  ...
  map(r,colbar=list(breaks=seq(-1,1,by=0.1),pal='t2m',rev=TRUE),type='fill')
  figlab('EOBS+ensemble mean MOS',ypos=0.999)
  dev.copy2pdf(file=paste('fig.D53Exp.cor.',
                 substr(results,14,nchar(results)-4),'.pdf',sep=''))
  print('Common EOFs')
  ...
  dev.copy2pdf(file=paste('fig.D53Exp.ceof.',
                 substr(results,14,nchar(results)-4),'.pdf',sep=''))
  plot(diagnose(ceof))
  figlab('EOBS+MOS')
    dev.copy2pdf(file=paste('fig.D53Exp.ceofdiagnostics.',
                 substr(results,14,nchar(results)-4),'.pdf',sep=''))
...
if (downscaledreanalysis) {
  mu.eof <- EOF(mu.eobs)
  plot(mu.eof,colbar=list(pal='t2m',breaks=seq(-0.02,0.02,by=0.001)),type='fill',rev=TRUE)
  figlab('EOBS',ypos=0.999)
  dev.copy2pdf(file='figSM.D53Exp.mu.eof.pdf')

## The wet-day frequency:
  
  fw.eof <- EOF(fw.eobs)
  dev.new()
  plot(fw.eof,colbar=list(pal='t2m',breaks=seq(-0.01,0.015,by=0.001)),type='fill',rev=TRUE)
  figlab('EOBS',ypos=0.999)
  dev.copy2pdf(file='figSM.D53Exp.fw.eof.pdf')
...
## retrieve did not get the right time stamp:
  es <- annual(C.C.eq(retrieve('air.mon.mean.nc',lon=c(-60,40),lat=c(20,70))))
...
## Additional diagnostics: canonical correlation analysis:
  ...
  plot(cca,colbar1=list(breaks=seq(-300,300,by=10)),colbar2=list(breaks=seq(-20,20,by=1)))
  figlab(expression(paste('NCEP/NCAR: ',e[s](T),'-slp')))
  dev.copy2pdf(file='figSM.D53Exp.es-slp.cca.pdf')
  ...
  plot(mu.ds,colbar1=list(breaks=seq(-0.025,0.025,by=0.005),rev=TRUE),
       colbar2=list(breaks=seq(-350,350,by=25)),type='fill')
  figlab('ESD(NCEP/NCAR)',ypos=0.999)
  dev.copy2pdf(file='fig.D53Exp.mu.ds.pdf')
  dev.new()
  plot(fw.ds,colbar1=list(breaks=seq(0.0,0.02,by=0.001),rev=TRUE),
       colbar2=list(breaks=seq(-20,20,by=2.5)),type='fill')
  figlab('ESD(NCEP/NCAR)',ypos=0.999)
  dev.copy2pdf(file='fig.D53Exp.fw.ds.pdf')
}
...
dev.copy2pdf(file='figSM.D53Exp.spectralpower-scales.pdf')

dev.new()
plot(attr(zst,'timeseries'),plot.type='single',col=colscal(20),main='T(2m)')
dev.copy2pdf(file='figSM.D53Exp.timeseries-scales.pdf')
...
dev.copy2pdf(file='figSM.D53Exp.spectralpower-scales_slp.pdf')

dev.new()
plot(attr(yst,'timeseries'),plot.type='single',col=colscal(20),main='SLP')
dev.copy2pdf(file='figSM.D53Exp.timeseries-scales_slp.pdf')
...
dev.copy2pdf(file='figSM.D53Exp.extremes-samplesize.pdf')
