...
## Generate a common EOF
  ...
  print("Create common EOF")
  if (mask) Z <- mask(Y,land=TRUE) else Z <- Y
  Z <- subset(Z,it=range(year(X)))
  str(Z); print(class(Z)); print(table(month(Z),year(Z)))
  str(X)
  nmembers <- length(table(month(X)))
  for (i in 1:nmembers) {
    print(month.abb[i])
    ## Months 1-10 are the different members
    z <- subset(X,it=month.abb[i])
    print(table(month(z),year(z)))
    index(z) <- year(z)
    Z <- combine(Z,z)
  }
  ceof <- EOF(Z)
