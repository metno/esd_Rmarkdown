...
### Dry conditions and wild fire statistics

Use the EOFs describing the dry conditions as predictors and wild fire statistics as predictand to assess the connection
...
Follow up with the residuals and the large-scale temperture 
...
Add the the two components to get an estimate of the total change. The `dse`objects are lists which contain `zoo` time series:
...
Model number of evens $n_f$ and their area $A_f$ seperately, a little like precipitation ($\mu$ and $f_w$). The area has an impact on the risk of affecting the number of assets. Base this on historic data and relate to large-scale conditions (predictors). Model as a partly stochastic process where the likelihood depends on an external variable. Poisson model (`GLM`) of $n_f$ and the nature of the $A_f$ distribution dictates the model strategy for the area.
