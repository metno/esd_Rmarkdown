...
First pre-amble that checks whether the esd-package is installed and installs it if needed.
...
**Functions which estimate various statistics**
...
The definition of functions is followed by code that carry out the analysis based on these.  First get the data needed:
...
Then the predictand data is processed: estimate annual mean aggregates and the mean seasonal cycle. Remove stations with large gaps of missing values. 
...
Get and process the predictor data.
