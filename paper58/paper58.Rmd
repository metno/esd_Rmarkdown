...
## But we use the data from the 1990s and 2000s in the rest of the analysis!
## It makes no sense to exclude it here, and the results are similar.
...
Figure SM1 shows a test of the assumption that that the wet-day daily precipitation is approximately exponentially distributed, by comparing the actual percentiles with quantiles estimated for different samples 
...
Figure SM2 presents the $R^2$ statistics from the model calibration (i.e., the regression between the seasonal cycles of $\mu$ and $e_s$) for different locations.
...
Figure SM3 compares the observed and predicted historical trends in the wet-day mean preciptation.
...
Figure SM4 shows the historical trends in the wet-day mean preciptation.
...
Figure SM5 displays a histogram of historical trends in the wet-day frequency for the period... for the stations in Europe shown in Figure SM6. 
...
Figure SM6 shows the spatial distribution of the historical trends in wet-day frequency on a map.
...
Figures SM7-8 require additional data and will follow after Figures SM9-11

##**Figure SM9**

Figure SM9 shows an example of projections of the wet-day mean for one location, showing all three emission scenarios.
...
Figure SM10 presents a map of the NCEP reanalysis temperature for the North Atlantic region that is used as predictor for the downscaling (see e.g., Figure 1 and 3). 
...
Figure SM11 show the seasonal cycles of various precipitation statistics (mean, wet-day mean, wet-day frequency, mean consecutive wet days and dry days), calculated based on ECA&D observations for stations with long precipitation records. 
...
col <- rgb((1+sin(pi*calmu$lat/180))/2,
           ...
           1-(1+sin(pi*calmu$lat/180))/2,0.45)
...
Figure SM9 compares the regression coefficients derived from the seasonal cycles of $\mu$ and the North Atlantic $\e_s$ (see e.g. Figures 1 and 3), to the regression coefficient from the comparison between the mean $\mu$ and $e_s$ from various sites (see Figure SM8).
