...
Collect aggregated GCM projections (CMIP5) and apply the regression models to the ensembles. 
...
t <- index(rcp4.5)
```
Extract statistics of the wet-daymean associated with different emission scenarios and time slices and calculate the changes from the present (2010) to the far future (2100).
...
## Figure SM5 is not used in either the main paper of supplementary material
<##**Figure SM5**>

#Figure SM5 is a histogram of the  shows the historical trends in the wet-day mean preciptation.
...
#     main='Wet-day percentile for annual maximum 24-hr precipitation',
#     xlab='p (%)')
#grid()
#figlab('Figure SM5')
#```
...
Figure SM6 displays a histogram of historical trends in the wet-day frequency for the period... for the stations in Europe shown in Figure SM7. 
...
Figure SM7 shows the spatial distribution of the historical trends in wet-day frequency on a map.
...
Load South American temperature and precipitation data from the climate database CLARIS and COST-VALUE(?). 
```{r}
## CLARIS
load('claris.Tx.rda')
load('claris.Pr.rda')
...
Combine and aggregate the observational data from North America and South America, and then fit a regression model based on comparing the annual mean $\mu$ and $e_s$ at the different stations.  
```{r}
