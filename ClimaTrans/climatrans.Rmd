...
### Function to evaluate spatio-temporal variance structure for SLP
...
Model the number with heavy rainfall (more than 50 mm/day) in Delhi and Bangalore. The results for Delhi and Bangalore also indicate a failure to capture the number of events. 
...
Also check whether the number of events in Delhi and Bangalore approximately follow a Poisson distribution.
...
The number of days with heavy rain is deviating somewhat from the Poisson distribution for the Indian data. We can compare the assumption of Poisson distribution with Norwegian data (all year) with known high quality (we have the metadata), and the results suggest a closer fit, although this data too is over-dispersed. 
...
```{r pca-of-residual-fw, fig.height=8,fig.width=8}
...
## Examine the common EOFs for SLP over the Indian sub-continent


Common EOFs can be used to assess the GCMs' ability to reproduce the spatio-temporal structure in covariance in cinnection to mean sea-level pressure (SLP) variations over the Indian sub-continent. In orther words, whether the GCMs reproduce the observed circulation features such as wind patterns associated with the monsoon. The code below compares the statistical character of the odel results.  

```{r fig.width=12, fig.height=20}
print('Compare common EOFs')
diag.slp <- biasdiag(slp,it=month.abb[season],verbose=FALSE)
mean.diff <- unlist(lapply(diag.slp,function(x) x$mean.diff[,1]))
sd.ratio <- 1 - unlist(lapply(diag.slp,function(x) x$sd.ratio[,1]))
ar.ratio <- 1 - unlist(lapply(diag.slp,function(x) x$autocorr.ratio[,1]))
summary(sd.ratio)
summary(ar.ratio)
```

This test shows a comparison of the standard deviation of the principal components (PCs) associated with the reanalysis SLP and the GCM SLP over the sub-indian continent (region shown in the maps above) and the year-to-year variability in the monsoon season mean SLP. A good reproduction of the spatio-temporal variance structure for the SLP is expected to yield leading EOFs with similar variance and hence the blue symbols should be close to zero for $1 - \rho_{gcm}/\rho_{ref}$ along the x-axis. The results suggest otherwise, with large differences in the standard deviation connected to the PCs, and indicate that all GCMs have difficulties reproducing the monsoon features in the monsoon SLP.  


## Example of station record

The format of the station records bears a structure from the past where it was more important to take up little space rather than being human readable. This cryptic and non-human readable way of recording data is associated several risks of error introduction, as it is easy to mistype some of the '0' entries and it is hard to spot obvious mistakes (e.g. unreasonable and unphysical values). Furthermore, reading data of this type of format also is prone to errors, as the data is stored in a way that may vary from time to time. There is also a high risk of the introduction of errors when reading such data. 
