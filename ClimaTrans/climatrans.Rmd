...
#### Function used to estimate proportional change (for precipitation):
...
```{r}
tas2es <- function(x,mask=FALSE,land=TRUE,season=6:9,FUN='min') {
...
save(file='~/Dropbox/Public/ClimaTrans/climatrans.pr.rda',climatrans.pr)
...
## Save the results for future use
save(file='~/Dropbox/Public/ClimaTrans/climatrans.tx.rda',climatrans.tx)
```
...
load('~/Dropbox/Public/ClimaTrans/climatrans.pr.rda')
...
```{r}
fw <- aggregate(subset(climatrans.pr,it=month.abb[5:10]),year,'wetfreq')
mu <- aggregate(subset(climatrans.pr,it=month.abb[5:10]),year,'wetmean')
q95 <- aggregate(subset(climatrans.pr,it=month.abb[5:10]),year,'quantile',probs=0.95)
...
if (!exists("es")) {
  ...
  #es <- aggregate(C.C.eq(subset(sst,it=month.abb[6:9])),year,'max')
  t2m <- retrieve('data/ncep/air.mon.mean.nc',lon=c(50,100),lat=c(5,30))
  es <- annual(t2m,'tas2es')
}
```
...
```{r}
if (!exists("slp")) {
  slp <- retrieve('data/ncep/slp.mon.mean.nc',lon=c(50,100),lat=c(0,40))
  slp <- aggregate(subset(slp,it=month.abb[6:9]),year,'mean')
}
...
plot(ds.mu,colbar1=list(breaks=seq(0,1,by=0.1),rev=2),
     colbar2=list(breaks=seq(-500,500,by=25),rev=TRUE))
plot(ds.fw)
```
...
Here include the argument \code{xfuns='tas2es'} to use the option \code{annual(tas2es(gcm))} rather than the option \code{annual(gcm,FUN='tas2es')} which will fail. Also  

```{r}
if (!file.exists('dse.mu.climatrans.rda')) {
  dse <- DSensemble(pca.mu,predictor=es,FUNX='tas2es',xfuns='tas2es',nmin=1)
  save(file='dse.mu.climatrans.rda',dse)
} else load('dse.mu.climatrans.rda')
dse.stations<-as.station(dse)
```
...
```{r}
  load('~/Dropbox/Public/ClimaTrans/climatrans.tx.rda')
...
```{r}
  predictor <- retrieve('data/ncep/air.mon.mean.nc',
                        param='air',lon=c(60,90),lat=c(10,35))
