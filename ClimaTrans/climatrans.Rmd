...
biasdiag <- function(predictor,param='slp',
                    path="CMIP5.monthly/",pattern="slp_Amon_ens_",type='ncdf',
                    period=c(1950,2015),rcp='rcp45',verbose=TRUE,
                    lon=c(0,100),lat=c(65,90),select=NULL,xlim=NULL) {

  ## Diagnose the common EOFs and effect of bias-adjustment

  it <- month.abb[as.numeric(rownames(table(month(predictor))))]
  ...
  d.y <- 6 # Number of leading EOFs to evalue
  years <- 1900:2100
  m <- length(years)
  months <- rep(month(y)[1],m)
  X <- matrix(rep(NA,N*m*d.y),N,m*d.y)
  dim(X) <- c(d.y[2],N,m)
  gcmnm <- rep("",N)
  ...
  for (i in 1:N) {
    ...
    gcmnm.i <- gsub('-','.',gcmnm.i)
    print(gcmnm.i)
    GCM <- aggregate(subset(gcm,it=month.abb[month(y)[1]]),year,FUN='mean') 
...
plot(1949:2004,ndhr50.1.cal$y,pch=19,ylab=expression(N(X > 100*mm)),xlab='',main=loc(ndhr)[1])
grid()
...
plot(1949:2004,ndhr50.3.cal$y,pch=19,ylab=expression(N(X > 100*mm)),xlab='',main=loc(ndhr)[3])
grid()
